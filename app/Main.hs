...
factk :: Integer -> (Integer -> t) -> t
factk 0 k = k 1
factk n k = factk (n-1) (\res -> k $ res * n)


--- ### `evenoddk :: [Integer] -> (Integer -> t) -> (Integer -> t) -> t`

evenoddk :: [Integer] -> (Integer -> t) -> (Integer -> t) -> t
evenoddk (x:[]) k1 k2 = if(mod x 2 == 0)
                          then k1 x
                        else
                          k2 x
evenoddk (x:xs) k1 k2 = if(mod x 2 == 0)
                          then evenoddk xs (\res -> k1 $ res + x) (\res-> k2 $ res)
                        else
                          evenoddk xs (\res -> k1 $ res) (\res-> k2 $ res + x)
...
isSimple (OpExp f a b) = isSimple a && isSimple b
isSimple (AppExp _ _) = False

--- ### `cpsExp :: Exp -> Exp -> Integer -> (Exp, Integer)`

cpsExp :: Exp -> Exp -> Integer -> (Exp, Integer)
cpsExp (IntExp e1) k n = (AppExp k (IntExp e1), n)
cpsExp (VarExp e1) k n = (AppExp k (VarExp e1), n)
cpsExp (IfExp e1 e2 e3) k n  = if (isSimple e1)
                                 then (IfExp e1 newExp1 newExp2, n2)
                               else
                                 cpsExp e1 (LamExp v (IfExp (VarExp v) newExp3 newExp4)) n5
                               where
                                (newExp1, n1) = cpsExp e2 k n
                                (newExp2, n2) = cpsExp e3 k n1
                                (v, n3) = gensym n
                                (newExp3, n4) = cpsExp e2 k n3
                                (newExp4, n5) = cpsExp e3 k n4


cpsExp (OpExp f e1 e2) k n = if (isSimple e1 && isSimple e2)
                               then (AppExp k (OpExp f e1 e2), n)
                             else if (isSimple e1 == False && isSimple e2)
                               then cpsExp e1 (LamExp v1 (AppExp k (OpExp f (VarExp v1) e2))) n1
                             else if (isSimple e1 && isSimple e2 == False)
                               then cpsExp e2 (LamExp v2 (AppExp k (OpExp f e1 (VarExp v2)))) n2
                             else
                               cpsExp e1 (LamExp v3 v5) n5
                             where
                              (v1,n1) = gensym n
                              (v2,n2) = gensym n
                              (v3,n3) = gensym n
                              (v4,n4) = gensym n3
                              (v5,n5) = cpsExp e2 (LamExp v4 (AppExp k (OpExp f (VarExp v3) (VarExp v4)))) n4


cpsExp (AppExp f e1) k n = if (isSimple e1)
                             then (AppExp (AppExp f e1) k, n)
                           else
...
cpsDecl :: Stmt -> Stmt
cpsDecl (Decl f myparams e1) = Decl f (myparams++["k"]) newExp1
                             where
                               (newExp1, n1) = cpsExp e1 (VarExp "k") 1